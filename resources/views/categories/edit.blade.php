...
<div class="form-container">
    ...
    <form action="{{ route('categories.update', $category) }}" method="POST" class="form">
        ...
        <div class="form-group">
            ...
            <input type="text" name="title" id="title" value="{{ $categoryLanguage->title }}" required class="form-control" placeholder="Ejemplo: Héroe">
        </div>
        <div class="form-group">
            <label for="lang_id" class="form-label">Idioma:</label>
            <select name="lang_id" id="lang_id" required class="form-control">
                @foreach ($languages as $language)
                    <option value="{{ $language->id }}" {{ $language->id == $categoryLanguage->lang_id ? 'selected' : '' }}>
                        {{ $language->code }}
                    </option>
                @endforeach
            </select>
